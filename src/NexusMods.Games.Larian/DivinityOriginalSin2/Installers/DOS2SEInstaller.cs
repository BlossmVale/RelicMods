...
namespace NexusMods.Games.Larian.DivinityOriginalSin2.Installers;
...
/// </summary>
public class DOS2SEInstaller : ALibraryArchiveInstaller
{
    public DOS2SEInstaller(IServiceProvider serviceProvider) :
        base(serviceProvider, serviceProvider.GetRequiredService<ILogger<DOS2SEInstaller>>())
    {
    }

    private const string DOS2SEFileName = "DWrite.dll";
        ...
        CancellationToken cancellationToken)
    {
        ...
        var nodes = tree.FindSubPathsByKeyUpward([DOS2SEFileName]);
        if (!nodes.TryGetFirst(out var dllNode)) return ValueTask.FromResult<InstallerResult>(new NotSupported(Reason: $"Found no file named `{DOS2SEFileName}`"));
