...
using NexusMods.Games.Larian.DivinityOriginalSin2.Utils.LsxXmlParsing;
using NexusMods.Hashing.xxHash3;
using NexusMods.Games.Larian.DivinityOriginalSin2.Utils.PakParsing;
...
namespace NexusMods.Games.Larian.DivinityOriginalSin2.Emitters;
...
public class DependencyDiagnosticEmitter : ILoadoutDiagnosticEmitter
{
    ...
    public DependencyDiagnosticEmitter(IServiceProvider serviceProvider, ILogger<DependencyDiagnosticEmitter> logger, IOSInformation os, IGameDomainToGameIdMappingCache mappingCache)
    {
        ...
        _os = os;
        _gameDomain = mappingCache[DivinityOriginalSin2.NexusModsGameId.Value];
    }
...
#region Helpers
    ...
    private static Optional<LoadoutFile.ReadOnly> GetScriptExtenderLoadoutFile(Loadout.ReadOnly loadout)
    {
        return LoadoutItem.FindByLoadout(loadout.Db, loadout).OfTypeLoadoutItemGroup()
            ...
            .SelectMany(group => group.Children.OfTypeLoadoutItemWithTargetPath()
                .OfTypeLoadoutFile()
                .Where(file => file.AsLoadoutItemWithTargetPath().TargetPath.Item2 == Dos2Constants.BG3SEGamePath.LocationId &&
                               file.AsLoadoutItemWithTargetPath().TargetPath.Item3 == Dos2Constants.BG3SEGamePath.Path
                )
        ...
        bool onlyEnabledMods)
    {
        return LoadoutItem.FindByLoadout(loadout.Db, loadout)
            ...
            .SelectMany(group => group.Children.OfTypeLoadoutItemWithTargetPath()
                .OfTypeLoadoutFile()
                .Where(file => file.AsLoadoutItemWithTargetPath().TargetPath.Item2 == Dos2Constants.ModsLocationId &&
                               file.AsLoadoutItemWithTargetPath().TargetPath.Item3.Extension == Dos2Constants.PakFileExtension
                )
            )
            .ToArray();
    }
